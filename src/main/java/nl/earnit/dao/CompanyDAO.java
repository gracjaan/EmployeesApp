...
public class CompanyDAO extends GenericDAO<User> {
    ...
    public List<UserResponse> getStudentsForCompany(String companyId) throws SQLException {
        String query = "SELECT u.id, u.first_name, u.last_name, u.last_name_prefix, u.type, u.email, u.btw, u.kvk, u.address FROM user u, company_user c WHERE u.id = c.user_id AND c.company_id = ?";
        PreparedStatement statement = this.con.prepareStatement(query);
